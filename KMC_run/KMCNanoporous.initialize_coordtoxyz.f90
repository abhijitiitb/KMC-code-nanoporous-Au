...
! Preparing AL1 the coordinates of particle
   ...
   CALL Loadfile(AL,filename,NAtoms_input)   !nanoparticle
   !crystaltype="fcc"
   !filename="np.xyz"
   ...
   cm=0._dp
   DO i=1,AL%NAtoms
      cm=cm+AL%AtomCoord(3*i-2:3*i)
   END DO
   cm=cm/REAL(AL%NAtoms)
