...
MODULE LJSubroutines

   USE VARIABLE_TYPE
   IMPLICIT NONE
   INTEGER, PARAMETER :: maxatom=20000
   REAL(dp), DIMENSION(maxatom) :: DensityArray
   REAL(dp), DIMENSION(3*maxatom) :: DensityArray1
   
   CONTAINS
   
   !xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx  Only energy evaluation xxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
   SUBROUTINE LJEnTaylor(AL,NAtoms) !LJ Energy Generic (n species) Taylor
      IMPLICIT NONE
      ...
      MaxPairPotentialTableSize=MaxLJTableSize
   ...
   END SUBROUTINE LJForceLinearInterp
   !xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
   
END MODULE LJSubroutines
