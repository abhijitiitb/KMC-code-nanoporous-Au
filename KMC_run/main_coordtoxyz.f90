...
!  USE mdmpi_domaindecomposition
   ...
   REAL(dp) :: time,TargetPrintTime,fracdiffusion,kmcx,kmcd,composition,mincoord(3)
   REAL(dp) :: boxsize_x=90.0,boxsize_y=90.0,boxsize_z=90.0     !dimensions for AL%BoxSize
   ...
   ! CALL LatchParticle(AL1,AL) !from here on, AL is latched particle alongwith the vacant sites
   ...
   ! CALL Store_local_composition(AL)
   ! CALL Initialize_KMC
